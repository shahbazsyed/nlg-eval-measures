...
def _JS_divergence(peer_distribution, model_distribution):
    average = _compute_average_of_two_distributions(
        peer_distribution, model_distribution)
    js = (_KL_divergence(peer_distribution, average) +
          _KL_divergence(model_distribution, average)) / 2.
...
class JsdScorer():
    ...
    def compute_score(self, peer, model, as_ngrams=2):
        peer_distribution = preprocessing.compute_term_frequency_in_text(
            peer, as_ngrams)
        model_distribution = preprocessing.compute_term_frequency_in_text(
            model, as_ngrams)
        avg = 0.
